  ...
  public function update(Request $request, $id)
{
    ...
    $messages = [
        ...
        'logo_url.image' => 'Logo must be an image.',
        'logo_url.mimes' => 'Logo must be a file of type: jpeg, png, jpg, gif.',
        'store_name.required_if' => 'Store name is required for owners.',
        'description.required_if' => 'Description is required for owners.',
    ];
    ...
    $rules = [
        ...
        'store_name' => 'required_if:role,owner',
        'description' => 'required_if:role,owner',
    ];
    ...
    if ($request->hasFile('profile_picture')) {
        if ($profilePicturePath && file_exists(public_path('storage/profile/' . $profilePicturePath))) {
            unlink(public_path('storage/profile/' . $profilePicturePath));
        }
        $profilePicturePath = uniqid() . '-' . $request->profile_picture->getClientOriginalName();
        $request->profile_picture->move(public_path('storage/profile'), $profilePicturePath);
    }

    // Handle Store & Logo if role is owner
    $logoPath = null;
    if ($request->role === 'owner') {
        $store = $user->store;
        ...
        if ($request->hasFile('logo_url')) {
            if ($logoPath && file_exists(public_path('storage/logo/' . $logoPath))) {
                unlink(public_path('storage/logo/' . $logoPath));
            }
            $logoPath = uniqid() . '-' . $request->logo_url->getClientOriginalName();
            $request->logo_url->move(public_path('storage/logo'), $logoPath);
        }
